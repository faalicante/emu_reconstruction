...
void DrawMos(int pl, int brick, float kRun, const char* outdir, const char* seg="0.0")
{
  ...
  TFile *f = TFile::Open(Form("p%03i/%i.%i.%s.mos.root", pl, brick, pl, seg), "READ");
  gErrorIgnoreLevel = kUnset;
  
  if (!f || f->IsZombie()) {
    printf("DrawMos: Skipping plate %d (no .mos.root file)\n", pl);
    if (f) delete f;
    return;
  }
  ...
  if (!couples) {
    printf("DrawMos: Skipping plate %d (no couples tree)\n", pl);
    delete f;
    return;
  }
  ...
  if(gROOT->IsBatch()) c->SaveAs(Form("%s/mos%2.2d.png", outdir, pl));

}

void check_mos(int lastplate, int firstplate, int brick, float kRun, const char* outdir, const char* seg="0.0") {
  for (int plate = firstplate; plate <= lastplate; plate++){
    DrawMos(plate, brick, kRun, outdir, seg);
  }
}
